...
density2 <- read.table("density_R2.csv", header= T, sep = ";", dec=",", strip.white = T)
attach(density)

Temp <- read.table("temperature.csv", header= T, sep = ";", dec=".", strip.white = T)
attach(Temp)
...
P03D01 <- subset(nema, Tratamento=="Porco" & Replica==3 & Dia==1)
P03D02 <- subset(nema, Tratamento=="Porco" & Replica==3 & Dia==2)
P03D03 <- subset(nema, Tratamento=="Porco" & Replica==3 & Dia==3)
P03D04 <- subset(nema, Tratamento=="Porco" & Replica==3 & Dia==4)
P03D05 <- subset(nema, Tratamento=="Porco" & Replica==3 & Dia==5)
P03D06 <- subset(nema, Tratamento=="Porco" & Replica==3 & Dia==6)
P03D07 <- subset(nema, Tratamento=="Porco" & Replica==3 & Dia==7)
...
D_P02D01 <- data.frame(summary(P02D01$Familia))
...
FF_P02D01 <- data.frame(table(P02D01$Familia))
...
write.xlsx(tabela, "table_primer2.xlsx", row.names = FALSE)
...
log.density = log(density2$Density)
density2 = data.frame(density2,log.density)

density = data.frame(density)
P_graph <- density[1:28,]
Plog <- subset(density, Tratamento == "Pig")
...
t = bwplot(Dia~log.density | Tratamento)


bwplot(Dia~log.density | Tratamento, pch=16, col=1,
       ylab="Day", xlab="Density in log per 100g of soil")

bwplot(Dia~Density | Tratamento, pch=16, col=1,
       ylab="Day", xlab="Density in log per 100g of soil")


m = tapply(density$Density, density$Tratamento, mean)
v = tapply(density$Density, density$Tratamento, var)
plot(v~m)
cor.test(m,v)

qqnorm(density2$log.density)
qqline(density2$log.density)

shapiro.test(density2$log.density)

friedman.test(log.density|Tratamento, data = density2)
#write.xlsx(density2, "density_R2.xlsx")

#Temperature

bwplot(Day~Temperature|Treatment)
m = tapply(Temp$Temperature, Temp$Day:Temp$Treatment, mean) 

temp.pig= subset(Temp, Treatment=="Pig")
temp.cont= subset(Temp, Treatment=="Control")
temp.fake= subset(Temp, Treatment=="Fake")
MTP=tapply(temp.pig$Temperature,temp.pig$Day,mean)
SDTP=tapply(temp.pig$Temperature,temp.pig$Day,var)
MTC=tapply(temp.cont$Temperature,temp.cont$Day,mean)
SDTC=tapply(temp.cont$Temperature,temp.cont$Day,var)
MTF=tapply(temp.fake$Temperature,temp.fake$Day,mean)
SDTF=tapply(temp.fake$Temperature,temp.fake$Day,var)

temp.table <- cbind(MTP,SDTC,MTC,SDTP,MTF,SDTF)

bwplot(Day~Temperature | Treatment, pch=16, col=1,
       ylab="Day", xlab="Density in log per 100g of soil")
xrange <-range(-1:19)
yrange <-range(22:40)

plot(Day, Temperature, type = "n", xlab = "Day", ylab = "Temperature (ºC)")
lines(temp.pig$Temperature, type="n", col="red")


#write.xlsx(t2.table, "forensic_ninja.xlsx")

qqnorm(Temp$Temperature)
qqline(Temp$Temperature)
shapiro.test(temp.table$temperature)

log.temperature= log(Temperature)
temp.table= data.frame(Temp, log.temperature)
attach(temp.table)

x=Anova(dayModel,idata = dayFrame, idesign =~dayFactor,type="III"